...
describe('Auth Service', () => {
  ...
  describe('constructor()', () => {
    it('should be initialized', inject(
      [AuthService, ErrorDisplayService, Router], (service: AuthService) => {
  ...
  describe('login()', () => {
    it('should invoke Http.post with the login data in order to log user in', inject(
      [AuthService, ErrorDisplayService, Router], (service: AuthService) => {
  ...
  describe('register()', () => {
    it('should invoke Http.post with the user data in order for the user to sign up', inject(
      [AuthService, ErrorDisplayService, Router], (service: AuthService) => {
        let spy  = spyOn(service['http'], 'post').and.callFake(() => {
          return Observable.empty();
        });
  ...
  describe('logout()', () => {
    it( `should clear the 'token key' and 'name key' from local storage ` +
        `and redirect the user to the login page`, inject(
      [AuthService, ErrorDisplayService, Router], (service: AuthService) => {
  ...
  describe('_authenticate()', () => {
    it(`should display an error message if the server does not return a token`, inject(
      [AuthService, ErrorDisplayService, Router], (service: AuthService) => {
    ...
    it( `should save the token and the firstname in ` +
        `localstorage if the server does return a token`, inject(
      [AuthService, ErrorDisplayService, Router], (service: AuthService) => {
        let spy  = spyOn(service['router'], 'navigate');
